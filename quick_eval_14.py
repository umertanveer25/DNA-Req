...
print('--- Phase 0 (TF-IDF Baseline) 14 Algorithms (10-Fold CV) ---')
...
for name, clf in algorithms.items():
    ...
    res0[name] = np.mean(accs)*100
for k,v in sorted(res0.items(), key=lambda x:x[1], reverse=True):
    print(f'{k:25}: {v:.2f}%')

print('\n--- Phase 2 (DNA+SBERT) 14 Algorithms (10-Fold CV) ---')
...
for name, clf in algorithms2.items():
    ...
    for tr, val in fold_pairs:
        ...
        clf_copy.fit(X_cv[tr], y[tr])
        accs.append(accuracy_score(y[val], clf_copy.predict(X_cv[val])))
    res2[name] = np.mean(accs)*100
for k,v in sorted(res2.items(), key=lambda x:x[1], reverse=True):
    print(f'{k:25}: {v:.2f}%')
