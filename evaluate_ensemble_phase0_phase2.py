...
def run_evaluation(X, y, splits):
    ...
    aggregated = {name: [] for name in get_ensemble_algorithms().keys()}
...
if __name__ == '__main__':
    ...
    print('\n' + '='*50)
    print(' EXTRACTING PHASE 0 (TF-IDF ENGLISH)')
    print('='*50)
    tf_vectorizer = TfidfVectorizer(max_features=482, sublinear_tf=True)
    X_phase0 = tf_vectorizer.fit_transform(texts).toarray()
    print(f'Phase 0 Matrix: {X_phase0.shape}')

    t0 = time.time()
    res_phase0 = run_evaluation(X_phase0, y, splits)
    print(f'\n--- Phase 0 Ensemble Results ({time.time()-t0:.1f}s) ---')
    for k, v in res_phase0.items():
        print(f'{k:20}: {v:.2f}%')

    print('\n' + '='*50)
    print(' EXTRACTING PHASE 2 (DNA + SBERT FUSION)')
    print('='*50)
    ...
    print(f'Phase 2 Matrix: {X_phase2.shape}')

    t0 = time.time()
    res_phase2 = run_evaluation(X_phase2, y, splits)
    print(f'\n--- Phase 2 Ensemble Results ({time.time()-t0:.1f}s) ---')
